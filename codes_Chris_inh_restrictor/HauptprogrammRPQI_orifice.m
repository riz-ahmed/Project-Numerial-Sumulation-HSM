% Hauptprogramm - Berechnung P(R),Q(R),I(R) - Orifice Restrictor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% !!! NICHT VALIDIERT!!!
...
% Dieses Programm berechnet den Düsentyp "Orifice Restrictor"
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Einschränkung von Belf13:
% (h0+delta)/(2r0) < 0.2, bitte beachten!
...
h0 = 13.4e-6; % Spalthöhe am Spalteingang in µm am Anfang der Fase
% Eingabe
    ...
    delta   = 5e-6;       % Taschentiefe ausgehend von h0, Höhe nach Tasche: h1 = h0-delta 
    r1      = r0 + 60e-6; % Außenradius der Tasche in m  
...
while ((pa - pa_stern_neu) < 0)
    ...
    for i = 0:9
        ...
        S=find(A(:,1)==p0);
        if (isempty(S))
            [R,P,Q,I,I_err,M0,re0stern] = RKV45_RPQI_orifice(ps,p0,T0,Rgas,kappa,my0,h0,delta,r0,r1,ra,eps);    
