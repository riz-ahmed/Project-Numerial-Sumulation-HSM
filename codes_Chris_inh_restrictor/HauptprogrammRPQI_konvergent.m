% Hauptprogramm - Berechnung P(R),Q(R),I(R) - Konvergent Restrictor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Dieses Programm berechnet den D¸sentyp "Konvergent Restrictor"
% Die einzelnen Hauptprogramme berechnen den jeweiligen D¸sentyp:
...
%       3. Restrictor mit Schr‰ge ¸ber ganze L‰nge
%          (konvergierender Spalthˆhe)
%           --> HauptprogrammRPQI_konvergent

%       4. Orifice Restrictor, Tasche im Eingangsbereich,
%          Einschr‰nkungen bzgl. Taschengeometrie beachten!
...
% Eingabe der restlichen Grˆﬂen
h0 = 13.4e-6; % Spalthˆhe am Spalteingang in µm am Anfang der Fase
% Eingabe
    ps      = 700000;     % Speiﬂedruck in Pa
    p00     = ps-1000;    % gew‰hlter Startwert am Spalteingang in Pa
    T0      = 293;        % Speiﬂetemperatur in K
    Rgas    = 287.058;    % Gaskonstante trockene Luft
    kappa   = 1.4;        % Isentropenexponent
    my0     = 17.8e-6;    % Viskosit‰t Luft in Pa*s
    r0      = 400e-6;      % Radius Speiﬂebohrung in  m 
    nue      = 0.38e-3; % Winkel des konvergierenden Spalts in rad
    ra      = 27.6e-3;     % Auﬂenradius Lagerspalt
    pa      = 101300;     % Umgebungsdruck
    eps     = 1e-9;       % Zul‰ssiger Fehler bei SW Steuerung  
...
while ((pa - pa_stern_neu) < 0)
    ...
    for i = 0:9
        ...
        S=find(A(:,1)==p0);
        if (isempty(S))
            [R,P,Q,I,I_err,M0,re0stern] = RKV45_RPQI_konvergent(ps,p0,T0,Rgas,kappa,my0,h0,nue,r0,ra,eps);    
...
%         disp(['pa_stern: ',num2str(pa_stern_neu)]); 
        ...
        % Um unnˆtige Berechnungen zu sparen wird die Berechnungsschleife
        % verlassen wenn das neu berechnete pa_stern wieder grˆﬂer wird.
    ...
    % Bei kleinen Spalten in p00=A(im-1,1) "-1" lˆschen;
    if h0<5e-6
        p00=A(im,1);
    else
        p00=A(im-1,1);
...
% Sortiere A gem‰ﬂ der ersten Spalte aufsteigend um ersten Wert ab p0_min zu finden
% der grˆﬂer pa ist
B=sortrows(A);
[c,i] = min(B);

% Finde erstes p0 grˆﬂer als das p0_min mit pa_stern ist grˆﬂer gleich pa
...
while (abs(pa_stern_neu-pa)>precision)
    ...
    % Bestimme das p0 das mit pa_stern n‰her an pa dran ist, denn dort wird
    ...
    for i=0:9    
        ...
        % Abbruchbedingung abh‰ngig davon ob man von der oberen Grenze
        % runter z‰hlt oder von der Unteren nach oben.
    ...
    % Finde erstes p0 mit pa_stern ist grˆﬂer gleich pa
    K=find(B(:,2)>=pa & B(:,1)>=B(i(2),1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Berechnung des Massenstroms nach Waumanns, function enth‰lt noch weitere 
% Formeln. Die Formel von Waumanns ist validiert, Tang68 passt
% einigermaﬂen, die klassische viskose Formel konnte nicht validiert werden
...
% Groﬂ Lambda Berechnung f¸r Pertubation-Method
